<template>
  <v-card class="dynamic-height">
    <v-card-title>
      <v-icon color="primary" class="me-2">mdi-draw</v-icon>
      제작처 도면 출도율
    ...
    <v-row>
      <v-col>
        <canvas id="bluepringStatus" style="width: 100%; height: 100%;"></canvas>
...
const createGaugeChart = () => {
  const ctx = document.getElementById("bluepringStatus")?.getContext("2d");
  if (!ctx) return;
