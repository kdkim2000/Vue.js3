<template>
  <v-card class="dynamic-height">
    <v-card-title>
      <v-icon color="primary" class="me-2">mdi-truck-fast</v-icon>
      자재 입고율
      <v-icon color="green" class="me-2">mdi-circle</v-icon>
    </v-card-title>
    <v-row>
      <v-col>
        <canvas id="materialArrivalRate" style="width: 100%; height: 100%;"></canvas>
      </v-col>
    </v-row>
  </v-card>
</template>
...
const createProgressChart = () => {
  const ctx = document.getElementById("materialArrivalRate").getContext("2d");
