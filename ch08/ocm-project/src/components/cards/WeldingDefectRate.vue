<template>
  <v-card class="dynamic-height">
    <v-card-title>
      <v-icon color="primary" class="me-2">mdi-alert-circle</v-icon>
      누락 용접불량율
      <v-icon color="green" class="me-2">mdi-circle</v-icon>
    </v-card-title>
    <v-row>
      <v-col class="me-4" align="end">
        <canvas id="weldingDefectRate" style="width: 100%; height: 100%;"></canvas>
      </v-col>
    </v-row>
  </v-card>
</template>
...
const createComboChart = () => {
  const ctx = document.getElementById("weldingDefectRate").getContext("2d");
